...
tests = test
  [ "parseChar" ~: do
      ...
      runParser p "def"  @?= Left (Diagnostic Error "Expected 'a' but found 'd' at: :1:1" (SourcePos "" 1 1))
      runParser p ""     @?= Left (Diagnostic Error "Expected 'a' but found EOF at: :1:1" (SourcePos "" 1 1))
  , "parseAnyChar" ~: do
      runParser (parseAnyChar "bca") "abcd" @?= Right ('a', "bcd")
      runParser (parseAnyChar "xyz") "abcd" @?= Left (Diagnostic Error "Expected one of 'xyz' but found 'a' at: :1:1" (SourcePos "" 1 1))
  ...
  , "parseOr" ~: do
      ...
      runParser p "xyz"  @?= Left (Diagnostic Error "Expected 'b' but found 'x' at: :1:1" (SourcePos "" 1 1))
  , "parseAnd" ~: do
      let p = parseChar 'a' >>= \a -> parseChar 'b' >>= \b -> return (a, b)
      runParser p "abcd" @?= Right (('a', 'b'), "cd")
      runParser p "bcda" @?= Left (Diagnostic Error "Expected 'a' but found 'b' at: :1:1" (SourcePos "" 1 1))
      runParser p "acd"  @?= Left (Diagnostic Error "Expected 'b' but found 'c' at: :1:2" (SourcePos "" 1 2))
      runParser p "xyz"  @?= Left (Diagnostic Error "Expected 'a' but found 'x' at: :1:1" (SourcePos "" 1 1))
  , "spaces" ~: do
  ...
  , "parseSome" ~: do
      ...
      runParser digits "foobar42" @?= Left (Diagnostic Error "Expected one of '0123456789' but found 'f' at: :1:1" (SourcePos "" 1 1))
  , "parseUInt" ~: do
      ...
      runParser parseUInt "foobar42" @?= Left (Diagnostic Error "Expected one of '0123456789' but found 'f' at: :1:1" (SourcePos "" 1 1))
  , "parseInt" ~: do
